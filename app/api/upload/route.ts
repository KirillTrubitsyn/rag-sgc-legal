      ...
      messages: [
        {
          ...
          content: [
            ...
            {
              type: 'text',
              text: `Распознай и извлеки ВЕСЬ текст с этого изображения документа.

ВАЖНЫЕ ИНСТРУКЦИИ:
- Сохраняй оригинальную структуру текста (абзацы, списки, таблицы)
- Если документ содержит таблицы, представь их в текстовом формате
- Если есть подписи, печати или штампы - опиши их кратко в скобках
- Если текст плохо читаем, укажи [неразборчиво]
- Не добавляй свои комментарии, только извлеченный текст
- Язык текста: русский (если не указано иное)
      ...
      messages: [
        {
          ...
          content: [
            ...
            {
              type: 'text',
              text: `Проанализируй этот документ и извлеки из него весь текст.

Файл: ${filename}

ИНСТРУКЦИИ:
1. Извлеки весь текст из документа
2. Сохраняй структуру (заголовки, абзацы, списки, таблицы)
3. Если это скан или фото документа - распознай текст
4. Для таблиц сохраняй табличный формат
5. Укажи тип документа в начале (договор, акт, доверенность и т.д.)

Верни полный текст документа.`,
            },
          ],
        },
      ],
